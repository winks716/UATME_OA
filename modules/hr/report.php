...
switch($A){
	...
	case 'get.employee.overtime':
		...
		if($result->num_rows > 0){
			...
			while($array = $result->fetch_assoc()){
				$msg .= '<tr><td>'.$array['date'].'</td><td>'.$array['location'].'</td><td>'.$array['period'].'</td><td>'.$status[$array['status']]['namezh'].'</td><td>'.$array['reason'].'</td></tr>';
			}
			$msg .= '</table>';
			$httpstatus = 200;
		}else{
			$msg = '<table><tr><td>无加班申请记录</td></tr></table>';
			$httpstatus = 200;
		}
		sendResponse($httpstatus, $error, $msg);
	    break;
	case 'hr.get.employee.overtime':
		...
		if($result->num_rows > 0){
			$msg = '<table><tr><th class="span-2">日期</th><th class="span-5">地点</th><th class="span-2">时长</th><th class="span-4">状态</th><th>事由</th></tr>';
			while($array = $result->fetch_assoc()){
				$msg .= '<tr><td>'.$array['date'].'</td><td>'.$array['location'].'</td><td>'.$array['period'].'</td><td>'.$status[$array['status']]['namezh'].($array['status']==1 ?'<span class="clickbtn cancelApply" i="'.$array['id'].'" ei="'.$_POST['id'].'">[撤销]</span>':'').'</td><td>'.$array['reason'].'</td></tr>';
			}
			$msg .= '</table>';
			$httpstatus = 200;
		}else{
	...
	case 'overtime.apply.manager.report':
		...
		$assign['employee'] = $employee;
		$smarty->assign($assign);
		$smarty->display('hr/overtime.manager.report.html');
	    break;
